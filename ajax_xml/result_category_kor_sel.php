...
if($_POST["step"]==1) {
	...
	$bSql = mysql_query("SELECT catecode,catename FROM resultCategoryKor WHERE view_state='Y' AND catedeg='2' AND SUBSTRING(catecode,1,4)='".substr($resultCategory,0,4)."' ORDER BY listorder ASC, catecode ASC",$dbconn);
	...
	$cSql = mysql_query("SELECT catecode,catename FROM resultCategoryKor WHERE view_state='Y' AND catedeg='3' AND SUBSTRING(catecode,1,8)='".substr($resultCategory,0,8)."' ORDER BY listorder ASC, catecode ASC",$dbconn);
