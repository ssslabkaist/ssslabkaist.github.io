...
if($v_member_id=="admin") {
	//$v_name="<img src=\"/img/icon/admin.gif\" border=\"0\">";
	$v_name=$v_name;
}else{
	$v_name=$v_name;
}
...
<script src="/js/program.js"></script>
		...
		<div class="view_normal_head">
            ...
            <div class="data">
                <p><?if($v_email && $_SESSION["session_usr_id"]){ echo "<a href=\"mailto:".$v_email."\">".$v_name."</a>"; } else { echo "".$v_name.""; }?>&nbsp; ㅣ &nbsp;<?=$v_writedate?>&nbsp; ㅣ &nbsp;조회:<?=$v_ref?>
<?/*				
			...
			if(preg_match("/".$boardUsrAuth."/",$delete_auth) || $_SESSION["session_admin_id"]) {
				if($v_member_id) {		//삭제
					if($_SESSION["session_usr_id"]) {
						if($v_member_id==$session_usr_id) {
							echo "
				<li><a class=\"btn bg_red\" href=\"#void\" onclick=\"DEL('./process_board.php?mode=delView&".$subquery."&uid=".$v_uid."&cpage=".$cpage."&spage=".$spage."');\">DELETE</a></li>";
						}
					}
				}else{
					echo "
...
<?
$prevSql = mysql_query("SELECT * FROM `".$tn."` WHERE sid > '".$v_sid."' ORDER BY sid ASC LIMIT 1",$dbconn);
$prevRow = mysql_fetch_array($prevSql);

if($prevRow["uid"]) {
	echo "
				<span><a class=\"split\" href=\"./view.php?uid=".$prevRow["uid"]."&gid=".$prevRow["gid"]."&cpage=".$cpage."&spage=".$spage."&".$subquery."\">".stripSlashNotUsed($prevRow["subject"])."</a></span>";
}else{
	echo "
				<span><a class=\"split\" href=\"#void\">처음입니다</a></span>";
}
?>
			</div>
			<div class="h20"></div>
...
<?
$nextSql = mysql_query("SELECT * FROM `".$tn."` WHERE sid < '".$v_sid."' ORDER BY sid DESC LIMIT 1",$dbconn);
$nextRow = mysql_fetch_array($nextSql);

if($nextRow["uid"]) {
	echo "
				<span><a class=\"split\" href=\"./view.php?uid=".$nextRow["uid"]."&gid=".$prevRow["gid"]."&cpage=".$cpage."&spage=".$spage."&".$subquery."\">".stripSlashNotUsed($nextRow["subject"])."</a></span>";
}else{
	echo "
				<span><a class=\"split\" href=\"#void\">마지막입니다</a></span>";
}
?>
			</div>
		</div>
