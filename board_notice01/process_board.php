...
if($_POST["mode"]=="delPwd") {
	...
	$query = mysql_query("SELECT sid,pwd,upfile,gid,level,upfile2,upfile3 FROM `".$tn."` WHERE uid='".$uid."' AND (pwd='".$pass."' OR pwd=PASSWORD('".$pass."'))", $dbconn);
	if(!$query) {
		echo("query_error");
	}
...
if($_GET["mode"]=="delView") {
	...
	$query = mysql_query("SELECT sid,member_id,upfile,gid,level,upfile2,upfile3 FROM `".$tn."` WHERE uid='".$uid."'", $dbconn);
...
if ($_POST["mode"]=="delList") {
	...
	if ($uUid) {
		for ($k=0;$k<sizeof($uUid);$k++) {
			...
			$query = mysql_query("SELECT sid,pwd,upfile,uid,upfile2,upfile3 FROM `".$tn."` WHERE uid='".$uUid[$k]."'", $dbconn);
			if(!$query) {
				echo("query_error");
			}
...
if ($_POST["mode"]=="endWrite") {
	if($write_auth) {
		if(!preg_match("/".$boardUsrAuth."/",$write_auth) && (!$_SESSION["session_admin_id"] || $adminAuth!="Y")) {
			ERROR_BACK("접근 권한이 없습니다.");
		}
	}
...
if ($_POST["mode"]=="endReply") {
	...
	baReplyAuthKor($tn,$_POST["uid"]);		//reply.php 접근 및 답변 권한
...
//--------------------------------------------------------E N D-------------------------------------//
	...
	$r_level = $_POST['level']+1;
	$ref = 1;

	$sql = mysql_query("UPDATE `".$tn."` SET sid=sid+1 WHERE sid >= '".$_POST['r_sid']."'", $dbconn);
	if(!$sql) {
		echo("query_error");
	}
	...
	$query ="INSERT INTO `".$tn."` SET ";
	$query.="sid='".$_POST['r_sid']."',";
	$query.="gid='".$_POST['r_gid']."',";
	$query.="level='".$r_level."',";
...
//비밀글 체크
if($_POST["mode"]=="endSecret") {
	$sql = mysql_query("SELECT uid,pwd,gid FROM `".$tn."` WHERE uid='".$uid."'",$dbconn);
	$row = mysql_fetch_row($sql);

	if($row[1]==$pwd){
		$_SESSION["inquire_gid"]=$row[2];
		$_SESSION["inquire_pwd"]=$row[1];

		echo "
			<script language='javascript'>
				parent.location.href='./view.php?uid=".$uid."&gid=".$gid."&cpage=".$cpage."&spage=".$spage."&".$subquery."';
				parent.passwdChkClose();
				window.close();
			</script>";


		//winCloseTgtReload2("./view_ad.html?uid=".$uid."&cpage=".$cpage."&spage=".$spage."&".$subquery."");
	}else{
